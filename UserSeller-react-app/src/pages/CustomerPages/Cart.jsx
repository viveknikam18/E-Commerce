...
import { Box, Typography, Button, Divider, Grid, AppBar, Toolbar, IconButton, Drawer, List, ListItem, ListItemButton, ListItemText, ListItemIcon } from '@mui/material'
import { decrementQty, incrementQty, removeItem } from '../../reducswork/CartSlice'
  ...
  >
    <Grid container spacing={2} alignItems="center">
      ...
      <Grid item xs={12} sm={8} md={6}>
        <Typography><strong>Title:</strong> {item.productName}</Typography>
        <Typography><strong>Price:</strong> ₹{item.productPrice || 0}</Typography>
        <Typography><strong>Quantity:</strong> {item.quantity}</Typography>
        <Typography>
          <strong>Total:</strong> ₹{(item.productPrice || 0) * item.quantity}
        </Typography>
      </Grid>
      ...
      <Grid item xs={12} md={3}>
        <Box sx={{ display: "flex", gap: 1 }}>
          ...
          {/* Summary */}
          <Typography><strong>Total Quantity:</strong> {totalQuantity}</Typography>
          <Typography><strong>Total Amount:</strong> {totalAmount} </Typography>

          <Button sx={{backgroundColor:"blue", color:'black'}} onClick={()=>{navigate('/customer/checkout')}}>Checkout</Button>
        </>
      ) : (
